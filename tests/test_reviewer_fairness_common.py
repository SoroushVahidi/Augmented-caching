...
# Two additional tests exist upstream (on kbs/second-revision-science) that
# exercise scripts/experiments/run_reviewer_fairness.py and
# scripts/experiments/generate_fairness_certificate.py against this module.
# Those orchestration scripts are not part of this promotion slice (they
# belong to the larger fairness-protocol-runner surface, not the
# oracle-diagnostic / target-degeneracy diagnostics this file supports), so
# the tests that import them are intentionally omitted here rather than
# left failing against files that don't exist on this branch.
