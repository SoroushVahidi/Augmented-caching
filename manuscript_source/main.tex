...
This distinction can be illustrated with a simple case. Suppose two eviction candidates $q_1, q_2 \in \mathcal{V}_t$ have the same next-arrival time, so that a next-arrival predictor alone treats them as equally urgent to retain. Their behavior within the horizon can nonetheless differ: $q_1$ may be requested repeatedly after its first return, while $q_2$ is requested once and then not again within the horizon. Their finite-horizon losses can then differ, e.g.\ $L_H(q_1,t) = 3$ and $L_H(q_2,t) = 1$, so the target in \eqref{eq:eviction_loss} can distinguish evicting $q_2$ as preferable even though next-arrival prediction alone cannot distinguish the two candidates. Predicting when an object will return is not always equivalent to predicting the consequence of evicting it now. Restricting supervision to a finite horizon also avoids forcing the learner to resolve differences between very distant next arrivals, such as reuse at $t{+}1000$ versus $t{+}2000$, that fall outside the horizon and are not decision-relevant at time $t$.
