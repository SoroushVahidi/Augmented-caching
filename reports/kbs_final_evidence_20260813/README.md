...
Compact tracked summaries for the second-revision campaigns. For manuscript
locations and primary vs historical classification, start at
`docs/reviewer/START_HERE.md`. Status tags such as `SYNTHESIS_PENDING` in
older mapping files in this folder predate manuscript integration (2026-08-14)
and should not be read as unfinished experiments.
...
## Source campaign directories (canonical scientific record)

On the scientific working branch these trees remain the raw record. This
`main` reviewer package publishes the compact summaries in this folder plus
the audited Common-Model V2 and tie-aware oracle files. If a number here and
a number in a raw CSV ever disagree, the raw CSV is authoritative.

Listed for provenance (not all trees are copied onto `main`):
