...
| Concern | Status | Note |
|---|---|---|
| **Reviewer #2 Major 1** (learned-baseline comparison) | `EVIDENCE_SYNCED_SYNTHESIS_PENDING` | Corrected held-out `evict_value_v1` treatment (42/42) synced from Wulver and independently re-audited 2026-08-13 (`heldout_treatment_integrity.md`). No same-protocol comparison against LRU/SIEVE/FIFO/LRB/3L-Cache/HALP/CACHEUS exists — only `evict_value_v1` has true 42/42 same-protocol coverage; LRU/SIEVE/FIFO have a caveated different-run, window-matched supplementary comparison only; LRB/3L-Cache/HALP/CACHEUS have zero results under any protocol. Remaining work is manuscript synthesis, not new evidence. |
