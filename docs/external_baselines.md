...
- **Provenance**: Based on the official [songjiayang/LRB](https://github.com/songjiayang/LRB) logic, integrated via `scripts/experiments/reviewer/run_lrb_external_baseline.py`.
...
- **Status**: Independent reimplementation (Scientific branches).
- **Provenance**: Reimplemented based on the paper's description of the admission and eviction policies. Available in the `feat/3l-cache-baseline` worktree.

### 3. HALP
- **Paper**: *HALP: Heuristic-Aided Learned Caching* (OSDI '23).
- **Status**: Independent reimplementation / Adaptation (Scientific branches).
- **Note**: Reimplemented to follow the heuristic-augmented learning structure described in the paper. Available in the `feat/halp-baseline` worktree.

### 4. CACHEUS
- **Paper**: *CACHEUS: Machine Learning-based Caching for Block I/O* (FAST '21).
- **Status**: Author-released source (Scientific branches).
- **Provenance**: Integrated via `lafc.policies.external.cacheus`. Available in the `feat/cacheus-baseline` worktree.
...
```bash
python scripts/experiments/reviewer/run_lrb_external_baseline.py --capacities 32,64,128 --out-dir analysis/external_learned_baselines/lrb
```
