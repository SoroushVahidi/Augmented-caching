...
> Reviewer note (2026-08-14): use `docs/reviewer/START_HERE.md` as the
> current concern-to-manuscript map. This file is a working coverage ledger.
> Common-Model V2 and the tie-aware oracle **are** manuscript-integrated.
> Links to unpublished internal promotion queues are not part of the `main`
> reviewer package.
...
controls complete, audited, and integrated into the revised manuscript).
...
verified by this workstation (Wulver was not contacted directly). Internal
cross-environment promotion notes are not published on this `main` snapshot.
Local learning-curve fraction `50%` is now complete and
