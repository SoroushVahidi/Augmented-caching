...
| `scripts/experiments/run_cross_family_heldout_eval.py` | `MANUAL_SECTION_MERGE` (low effort) | 4-line diff: removes a hardcoded `$REPO_ROOT`-style workstation default for `--data-read-root` (portability fix). If Wulver has its own machine-local default in this file, both sides need reconciling to a portable default, not a blind overwrite. |
...
| `configs/reviewer_fairness_protocol.json` | `MANUAL_SECTION_MERGE` (low effort) | 1-line diff: removes a hardcoded `$REPO_ROOT`-style worktree path |
