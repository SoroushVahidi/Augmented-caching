...
| Held-out `evict_value_v1` retraining | `scripts/experiments/run_evict_cross_family_pipeline.py`, `scripts/experiments/run_cross_family_heldout_eval.py`, `docs/reviewer_fairness_cross_family_v1.md`, `scripts/analysis/prepare_r2_major1_evidence.py` | Corrected 42/42 CSV is `WULVER_ONLY_VALIDATED` but absent locally; local status in `analysis/kbs_r2_major1_evidence_prep_20260811/treatment_status.json` | `FINAL_COMPARISON_PENDING` locally / `WULVER_ONLY_VALIDATED` remotely | Usable for the primary table only after the exact Wulver CSV is synchronized and SHA-verified | Expected path `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/policy_comparison.csv`, SHA-256 `982bfdffdbd816b56c2eef86ecb730a1eb136b3f85e36ad533739e586fa0a296`; do not substitute old contaminated fair-window rows |
