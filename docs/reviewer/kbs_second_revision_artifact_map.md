...
|---|---|---|---|---|---|
| LRB | `scripts/experiments/run_reviewer_fairness.py`, `docs/reviewer_fairness_protocol.md`, `scripts/analysis/prepare_r2_major1_evidence.py` | `analysis/reviewer_fairness/policy_comparison_lrb.csv`; summary in `analysis/kbs_r2_major1_evidence_prep_20260811/` | `LOCAL_EXACT_PROTOCOL_VALIDATED` | Primary reviewer table: use only `policy_variant=primary_controlled_window` rows | `deployment_full_stream` rows are supporting context only; independent repository reimplementation |
| 3L-Cache | same fairness protocol | `analysis/reviewer_fairness/policy_comparison_three_l_cache.csv`; summary in `analysis/kbs_r2_major1_evidence_prep_20260811/` | `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_CAVEAT` | Primary reviewer table: controlled-window rows only | Fixed `batch_size=4096` default / `PASS_WITH_CAVEAT`; batch-size sensitivity belongs to supporting analysis, not the primary table |
...
| CACHEUS | same fairness protocol | `analysis/reviewer_fairness/policy_comparison_cacheus.csv`; summary in `analysis/kbs_r2_major1_evidence_prep_20260811/` | `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_PROVENANCE_CAVEAT` | Primary reviewer table: controlled-window rows only | Official-source status is recorded, but the external clone/provenance is not currently live-verifiable in this worktree; upstream fixed seed remains part of provenance |
...
| Held-out `evict_value_v1` retraining | `scripts/experiments/run_evict_cross_family_pipeline.py`, `scripts/experiments/run_cross_family_heldout_eval.py`, `docs/reviewer_fairness_cross_family_v1.md`, `scripts/analysis/prepare_r2_major1_evidence.py` | Corrected 42/42 CSV synced from Wulver 2026-08-13 and independently re-audited locally; status in `analysis/kbs_r2_major1_evidence_prep_20260811/treatment_status.json` and `reports/kbs_final_evidence_20260813/heldout_treatment_integrity.md` | `SCIENTIFICALLY_COMPLETE_SYNTHESIS_READY` (2026-08-13, later pass) | Usable as the primary artifact for `evict_value_v1` itself, and now as the reference for a validated exact-protocol comparison table against all seven baselines (see `major1_reviewer_summary.md`); `evict_value_v1` loses on a clear majority of matched cells against every one. An earlier version of this row claimed no comparison table exists; that was incorrect | Path `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/policy_comparison.csv` (present locally), SHA-256 `982bfdffdbd816b56c2eef86ecb730a1eb136b3f85e36ad533739e586fa0a296` (verified); do not substitute old contaminated fair-window rows |
...
| Item | Code / protocol | Current local output | Status | Eligibility | Caveats |
|---|---|---|---|---|---|
| Objective sweep (`eviction_loss`, `reuse_distance`, `next_arrival`, `pairwise`) | `docs/supervision_objective_ablation_protocol.md`, `scripts/experiments/run_supervision_objective_ablation.py` | `analysis/supervision_objective_ablation_v1/policy_comparison.csv` | `COMPLETE_VALIDATED` | Usable supporting evidence | Aggregate misses must be taken from the frozen CSV, not from stale notes |
...
| Controlled timing campaign | same protocol | `analysis/kbs_controlled_timing_20260810/raw_timing_runs.csv` (420/420 rows, synced from Wulver 2026-08-13, 13/13 transfer hashes PASS); `analysis/kbs_controlled_timing_final_analysis_20260811/` | `FINAL_VALIDATED_SYNCED` | Primary evidence for LRU/FIFO-Reinsertion/SIEVE/HALP-causal timing | Independently recomputed policy means matched exactly; `evict_value_v1`'s runtime remains a separate single-run measurement, not part of this 4-policy campaign -- see `reports/kbs_final_evidence_20260813/controlled_timing_interpretation.md` |
