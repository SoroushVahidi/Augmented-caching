...
`RUNNING`, `PENDING`, `IMPLEMENTATION_READY`, `SMOKE_ONLY`, `BLOCKED`,
`SUPERSEDED`, `INVALID_DO_NOT_USE`. Full detail and per-row provenance:
...
| Learned-baseline comparison (LRB / 3L-Cache / CACHEUS / causal-HALP) | Full protocol | `FINAL_VALIDATED` (with fidelity caveats) | Primary evidence; CACHEUS fidelity HIGH (official source), LRB/3L MEDIUM (independent reimpl), HALP LOW_TO_MEDIUM (no official public implementation) | `analysis/external_learned_baselines/` | High, with stated fidelity caveats per baseline |
