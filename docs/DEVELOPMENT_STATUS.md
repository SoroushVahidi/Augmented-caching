...
| Continuation-policy causal ablation (C0/C1/C2, frozen protocol) | -- | `IMPLEMENTATION_REPAIRED_SMOKE_VALIDATED` / `NOT_PRODUCTION_LAUNCH_READY` | Local smoke path now emits all three conditions: C0 LRU, C1 frozen `pi1`, C2 trained from frozen-`pi1` continuation labels. No scientific result yet. Remaining blocker: no audited production campaign runner/resume/output-writer exists locally; the Wulver-reported `reference_model=` caller was not present in any local source/history searched. The learning-curve closeout does not solve this Reviewer #3 item. | `src/lafc/continuation_policy_ablation.py`, `scripts/experiments/run_continuation_policy_causal_ablation_smoke.py`, `configs/continuation_policy_causal_ablation_v1.json` | None yet (not run at scale) |
...
| Continuation-policy causal ablation (C0/C1/C2) | -- | `IMPLEMENTATION_REPAIRED_SMOKE_VALIDATED` / `NOT_PRODUCTION_LAUNCH_READY` | Local repair added C0 LRU to the smoke output and validated the C1/C2 path. The unexpected-keyword defect is reproducible if `build_rollout_candidate_rows_v2(..., reference_model=...)` is called, but no local continuation production caller does that; the reported Wulver production runner is absent locally. **Not** `READY_TO_RUN` until a production runner is synced or built and audited. | n/a | `DO_NOT_PROMOTE`, see `WULVER_TO_GITHUB_PROMOTION_QUEUE.md` #9 |
...
3. Build or sync and audit the continuation-policy C0/C1/C2 production runner -- the local smoke is repaired, but a full campaign still needs a real runner/resume/output path and must not use the obsolete `build_rollout_candidate_rows_v2(..., reference_model=...)` interface.
...
- **Continuation-policy mismatch (H5/H6) is unresolved, not disfavored --
  ...
  **zero** of those 9 and worsened in 9 of the 12 overall. This is
  ...
  H5. The local smoke path is now `IMPLEMENTATION_REPAIRED_SMOKE_VALIDATED`,
  but it is still not production-launch-ready: no audited full campaign
  runner/resume/output path exists locally, and any Wulver-side runner using
  `build_rollout_candidate_rows_v2(..., reference_model=...)` still needs an
  explicit interface repair before use.
