...
| Corrected held-out `evict_value_v1` cross-family replay | Fairness protocol | `FINAL_VALIDATED_SYNCED` (2026-08-13) | 42/42 rows synced from Wulver, 16/16 transfer hashes PASS, independent local re-audit PASS; no same-protocol comparison table exists against LRU/SIEVE/FIFO/LRB/3L-Cache/HALP/CACHEUS | `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/`, `reports/kbs_final_evidence_20260813/heldout_treatment_integrity.md` | High for the 42/42 artifact itself; synthesis against baselines pending |
...
| Learned-baseline comparison (LRB / 3L-Cache / CACHEUS / causal-HALP) | Full protocol | Baseline side `FINAL_VALIDATED` (with fidelity caveats); treatment side `FINAL_VALIDATED_SYNCED` (2026-08-13); R2 Major 1 overall `EVIDENCE_SYNCED_SYNTHESIS_PENDING` | Primary baseline evidence; LRB is `LOCAL_EXACT_PROTOCOL_VALIDATED`, 3L-Cache is `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_CAVEAT`, CACHEUS is `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_PROVENANCE_CAVEAT`; corrected `evict_value_v1` CSV now synced and independently re-audited; final treatment-vs-baseline synthesis remains, but no same-protocol comparison exists to synthesize against LRU/SIEVE/FIFO/LRB/3L-Cache/HALP/CACHEUS | `analysis/reviewer_fairness/`, `analysis/kbs_r2_major1_evidence_prep_20260811/`, `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/` | High for baseline side; high for treatment artifact; synthesis-writing pending for both |
...
| Corrected held-out `evict_value_v1` (cross-family, 42/42) | -- | `FINAL_VALIDATED_SYNCED` (2026-08-13); local classification `EVIDENCE_SYNCED_SYNTHESIS_PENDING` | Complete on Wulver per relayed audit and now independently confirmed locally: 42/42 rows, 7 families x 3 capacities x 2 variants, all `ok`, SHA-256 `982bfdffdbd816b56c2eef86ecb730a1eb136b3f85e36ad533739e586fa0a296` (verified). Synced 2026-08-13 via `rsync` from `login02:/mmfs1/project/ikoutis/sv96/Augmented-caching`; independent local re-audit passed every structural/leakage/model-provenance gate. Old contaminated local `policy_comparison_evict_value_v1.csv` remains excluded and unused | `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/policy_comparison.csv` (synced, present locally); status manifest in `analysis/kbs_r2_major1_evidence_prep_20260811/treatment_status.json`; `reports/kbs_final_evidence_20260813/heldout_treatment_integrity.md`, `heldout_treatment_provenance.md` | Sync/verify done. Next: run `scripts/analysis/prepare_r2_major1_evidence.py --treatment-csv ...`, preserving the no-same-protocol-comparison caveat |
...
1. **DONE (2026-08-13).** Synced and independently re-audited the corrected
   ...
   Remaining R2 Major 1 work is manuscript synthesis only, preserving the
   no-same-protocol-comparison caveat.
2. Use the locally validated exact controlled-window LRB/3L-Cache/CACHEUS CSVs as the baseline side of that table; run `scripts/analysis/prepare_r2_major1_evidence.py --treatment-csv ...` now that the verified treatment CSV is synchronized. Sync Wulver jobs `1171965`-`1171967` later only for replication or if their missing config records a material difference.
