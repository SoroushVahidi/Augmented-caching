...
`controlled_timing_integrity.md`). Earlier same-day pass: C0/C1/C2 and
distribution-shift both completed and passed formal post-completion
integrity audit; see `reports/kbs_final_evidence_20260813/`. Prior consolidation 2026-08-11
...
| Corrected held-out `evict_value_v1` cross-family replay | Fairness protocol | `SCIENTIFICALLY_COMPLETE_SYNTHESIS_READY` (2026-08-13, later pass) | 42/42 rows synced from Wulver, 16/16 transfer hashes PASS, independent local re-audit PASS; exact-protocol comparison against all seven baselines (LRB/3L-Cache/CACHEUS/HALP/LRU/SIEVE/FIFO-Reinsertion) now validated -- `evict_value_v1` loses on a clear majority of matched cells against every one | `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/`, `reports/kbs_final_evidence_20260813/major1_reviewer_summary.md` | High for the 42/42 artifact and for the validated comparison; manuscript wording integration remains |
...
| Learned-baseline comparison (LRB / 3L-Cache / CACHEUS / causal-HALP) | Full protocol | Baseline side `FINAL_VALIDATED` (with fidelity caveats); treatment side `FINAL_VALIDATED_SYNCED` (2026-08-13); R2 Major 1 overall `SCIENTIFICALLY_COMPLETE_SYNTHESIS_READY` (2026-08-13, later pass) | Primary baseline evidence; LRB is `LOCAL_EXACT_PROTOCOL_VALIDATED`, 3L-Cache is `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_CAVEAT`, CACHEUS is `LOCAL_EXACT_PROTOCOL_VALIDATED_WITH_PROVENANCE_CAVEAT`; corrected `evict_value_v1` CSV now synced, independently re-audited, and validated against all seven baselines under an exact evaluation protocol (`major1_reviewer_summary.md`) -- `evict_value_v1` loses on a clear majority of matched cells against every one. An earlier note claiming no same-protocol comparison exists was incorrect | `analysis/reviewer_fairness/`, `analysis/kbs_r2_major1_evidence_prep_20260811/`, `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/`, `reports/kbs_final_evidence_20260813/` | High for baseline side, treatment artifact, and the validated comparison; manuscript wording integration remains |
| Exact-target oracle replication | 21 family-capacity cells, H=4 | `FINAL_VALIDATED` | Exact oracle beats LRU in 0/21 cells, ties 3/21, loses 18/21 | `analysis/exact_target_oracle_replication_v1/` | Do not rerun; generated output is local/ignored |
...
| Practical significance / controlled timing | -- | `FINAL_VALIDATED_SYNCED` (2026-08-13), implementation caveat unchanged | Synced from Wulver, 420/420 rows (7 families x 3 capacities x 4 policies x 5 repetitions), 13/13 transfer hashes PASS, policy means independently recomputed and matched; this is implementation timing evidence, not a theoretical complexity proof; `evict_value_v1`'s runtime is a separate single-run measurement, not part of this 4-policy table | `analysis/kbs_controlled_timing_20260810/`, `analysis/kbs_controlled_timing_final_analysis_20260811/`, `reports/kbs_final_evidence_20260813/controlled_timing_integrity.md` | High for the stated timing scope; payload present and hash/structure verified |
...
| Corrected held-out `evict_value_v1` (cross-family, 42/42) | -- | `SCIENTIFICALLY_COMPLETE_SYNTHESIS_READY` (2026-08-13, later pass) | Complete on Wulver per relayed audit and now independently confirmed locally: 42/42 rows, 7 families x 3 capacities x 2 variants, all `ok`, SHA-256 `982bfdffdbd816b56c2eef86ecb730a1eb136b3f85e36ad533739e586fa0a296` (verified). Synced 2026-08-13 via `rsync` from `login02:/mmfs1/project/ikoutis/sv96/Augmented-caching`; independent local re-audit passed every structural/leakage/model-provenance gate; compared under a validated exact evaluation protocol against all seven baselines (`evict_value_v1` loses on a clear majority of matched cells). Old contaminated local `policy_comparison_evict_value_v1.csv` remains excluded and unused | `analysis/reviewer_fairness_cross_family_v1/evict_value_v1_final_42_20260810/policy_comparison.csv` (synced, present locally); status manifest in `analysis/kbs_r2_major1_evidence_prep_20260811/treatment_status.json`; `reports/kbs_final_evidence_20260813/major1_reviewer_summary.md`, `heldout_treatment_integrity.md`, `heldout_treatment_provenance.md` | Sync, verify, and comparison all done. Only manuscript wording integration remains |
...
| Controlled timing campaign | -- | `FINAL_VALIDATED_SYNCED` (2026-08-13) | Wulver job `1171758`, complete. 420/420 rows (7 families x 3 capacities x 4 policies x 5 repetitions), 13/13 transfer hashes PASS. Mean per-request runtime independently recomputed and matched: LRU 4.680543us, FIFO-Reinsertion 5.168286us, SIEVE 9.523362us, HALP-causal 870.660257us (~186x LRU) | `analysis/kbs_controlled_timing_20260810/`, `analysis/kbs_controlled_timing_final_analysis_20260811/` (synced, present locally) | Synced and hash/structure verified; see `reports/kbs_final_evidence_20260813/controlled_timing_integrity.md` |
...
1. **DONE (2026-08-13).** Synced and independently re-audited the corrected
   held-out cross-family `evict_value_v1` replay (42/42, 16/16 transfer
   hashes PASS); see `reports/kbs_final_evidence_20260813/heldout_treatment_integrity.md`.
2. **DONE (2026-08-13, later pass).** Ran `scripts/analysis/prepare_r2_major1_evidence.py --treatment-csv ...`
   and independently re-validated its output against the raw CSVs (zero
   discrepancies); combined with an independently computed LRU/SIEVE/FIFO/
   HALP comparison, all seven baselines now have a validated exact-protocol
   comparison against `evict_value_v1`. See
   `reports/kbs_final_evidence_20260813/major1_reviewer_summary.md`.
   Remaining R2 Major 1 work is manuscript wording integration only. Sync
   Wulver jobs `1171965`-`1171967` later only for replication or if their
   missing config records a material difference.
...
6. **DONE (2026-08-13).** Synced and independently re-audited the controlled
   timing campaign (420/420, 13/13 transfer hashes PASS, policy means
   recomputed and matched); see
