...
**SUPERSEDED 2026-06-19.** The 11 items below are a *hypothetical/sample*
review the agent drafted before the real reviewer comments were available —
they were never claimed to be verbatim and were explicitly labeled
"sample review" at the time (see the Purpose line below, unedited). The
real, verbatim AE / Reviewer #2 / Reviewer #3 comments are now available in
`reports/kbs_real_reviewer_comments.md` and should be used instead for any
response-to-reviewers work. This file is kept for historical/provenance
reasons (it shows the internal-audit methodology applied per-item) and
because some of the underlying artifact-gap findings may still be useful
context, but **none of the items below should be treated as real reviewer
text.** Cross-check anything reused from here against
`reports/kbs_real_reviewer_comments.md` and
