...
def _family_summary_rows(rows: List[Dict[str, object]]) -> List[Dict[str, object]]:
    fam: Dict[str, Dict[str, float]] = {}
    for row in rows:
        family = str(row.get("family", "unknown"))
        slot = fam.setdefault(family, {"row_count": 0.0, "decision_ids": set(), "avg_regret_sum": 0.0})
        slot["row_count"] += 1.0
        slot["avg_regret_sum"] += float(row["rollout_regret_h"])
        slot["decision_ids"].add(str(row["decision_id"]))

    out: List[Dict[str, object]] = []
    for family, payload in sorted(fam.items()):
        row_count = int(payload["row_count"])
        out.append(
            {
                "family": family,
                "rows": row_count,
                "decisions": len(payload["decision_ids"]),
                "mean_rollout_regret": (payload["avg_regret_sum"] / row_count) if row_count else 0.0,
            }
        )
    return out
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build rollout-labeled candidate dataset for evict_value_v2")
    ...
    ap.add_argument("--output-dir", default="data/derived/evict_value_v2_rollout")
    ...
    cfg = EvictValueV2RolloutConfig(horizons=horizons, reference_policy=args.reference_policy)
    ...
    for trace_path in trace_paths:
        ...
        for capacity in capacities:
            ...
            if args.max_decisions_per_trace > 0:
                dec_ids = sorted({str(r["decision_id"]) for r in candidate_rows})
                rng.shuffle(dec_ids)
                keep = set(dec_ids[: args.max_decisions_per_trace])
    ...
    family_summary = _family_summary_rows(rows)
    _write_csv(out_dir / "family_summary.csv", family_summary)
    ...
    summary = {
        ...
        "family_count": len({str(r.get("family", "unknown")) for r in rows}),
