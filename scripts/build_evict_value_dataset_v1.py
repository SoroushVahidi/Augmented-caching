...
from lafc.evict_value_dataset_v1 import EvictValueDatasetV1Config, _split_by_trace_and_capacity, build_evict_value_examples_v1
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build direct candidate eviction-value dataset (v1)")
    ...
    cfg = EvictValueDatasetV1Config(horizons=horizons)

    rows: List[Dict[str, object]] = []
    for trace_name, (requests, _pages) in _iter_traces(sample_only=args.sample_only):
        for cap in capacities:
            rows.extend(build_evict_value_examples_v1(requests=requests, capacity=cap, trace_name=trace_name, cfg=cfg))
    ...
    val = splits["val"]
    test = splits["test"]
    if (not train or not val or not test) and rows:
        train = [r for i, r in enumerate(rows) if i % 7 not in {1, 2}] or rows
        val = [r for i, r in enumerate(rows) if i % 7 == 1] or rows[:1]
        test = [r for i, r in enumerate(rows) if i % 7 == 2] or rows[:1]

    out = Path(args.out_dir)
    _write_csv(out / "evict_value_v1_train.csv", train)
    _write_csv(out / "evict_value_v1_val.csv", val or train[:1])
    _write_csv(out / "evict_value_v1_test.csv", test or train[:1])
