...
from lafc.evict_decision_aligned_v1 import DecisionAlignedEvictConfig, build_evict_regret_examples_v1
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build regret-aligned eviction dataset (v1)")
    ...
    cfg = DecisionAlignedEvictConfig(horizon=args.horizon)
    ...
    _write_csv(out / "evict_regret_v1_train.csv", splits["train"] or rows[:1])
    _write_csv(out / "evict_regret_v1_val.csv", splits["val"] or rows[:1])
    _write_csv(out / "evict_regret_v1_test.csv", splits["test"] or rows[:1])

    summary = {
        "dataset": "evict_regret_v1",
    ...
    (out / "evict_regret_v1_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
