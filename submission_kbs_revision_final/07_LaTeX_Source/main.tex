...
\begin{abstract}
Caching with predictive information remains challenging because predictions must be translated into eviction decisions, not merely assessed for accuracy. We study online paging from a candidate-level perspective: at each full-cache miss, the policy scores the items currently in cache and evicts the candidate with smallest predicted downstream harm under a finite-horizon replay target built from a fixed continuation rule. We instantiate this framework as \texttt{evict\_value\_v1} in unweighted paging and evaluate it under a matched protocol against seven baselines spanning classical (LRU, SIEVE, FIFO-Reinsertion) and learned (LRB, 3L-Cache, CACHEUS, HALP) cache replacement, using identical traces, cache capacities, request budgets, and miss accounting. Under this matched comparison, \texttt{evict\_value\_v1} does not outperform any of the seven baselines, losing on a clear majority of matched cells against each. A full-pipeline ablation against reuse-distance, next-arrival, and pairwise-preference supervision likewise finds the eviction-loss instantiation worst or tied-worst of the four. A matched common-model control that changes only the training objective, however, does not support blaming the eviction-loss \emph{label} itself: eviction-loss is nominally best by total misses and is not materially worse than the alternatives. Complementary diagnostics show that the learned scorer agrees with the exact target on $97.5\%$ of decisions, while every measured non-LRU exact-oracle decision has multiple optimal actions (mean optimal-set fraction $\approx 0.991$). A deterministic exact-oracle replay loses to LRU on $18$ of $21$ cells, but this deficit is not robust to valid within-minimum tie-breaking: choosing the LRU-most minimizer never loses to LRU and incurs $413$ fewer total misses. A controlled continuation intervention shows a real but partial, family-dependent contribution, and a generic distribution-shift intervention does not itself improve online performance. Finally, a controlled wall-clock timing campaign shows a causal HALP reimplementation is roughly two orders of magnitude slower per decision than the lightweight baselines, while \texttt{evict\_value\_v1}'s own separately measured, single-run runtime is roughly four orders of magnitude slower than the lightweight baselines and substantially slower still than HALP, though it was not part of this controlled repeated-measurement campaign. Taken together, the results characterize finite-horizon eviction-value prediction as an empirically tested but currently unsuccessful supervision-target design whose leading supported limitation is action-underdetermined (tie-degenerate) supervision, rather than a deployment-ready replacement policy.
\end{abstract}
...
This paper studies that candidate-centric view, modeling each full-cache miss as a structured comparison among the current eviction candidates rather than as a single global trust decision about a predictive hint. Section~\ref{subsec:objective_scope} states the resulting research objective and scope.

\subsection{Research Objective and Scope}
\label{subsec:objective_scope}

Building on the candidate-centric premise introduced above, this paper formulates and directly tests a seemingly natural decision-aligned target for cache eviction -- the finite-horizon downstream cost of evicting each candidate -- and asks whether structural alignment with the eviction decision is, by itself, sufficient for a learned target to be useful, rather than assuming that alignment guarantees good online behavior.

We organize this investigation around three properties a supervision target for online eviction can possess. These are presented as the conceptual lens used in this study, not as formally established universal definitions. \emph{Decision alignment} means the supervision target corresponds directly to the downstream eviction action or cost, rather than to an indirect proxy that must still be translated into a decision. \emph{Decision informativeness} means the target provides enough separation among the currently available candidates to select meaningfully among them, rather than leaving the decision effectively unconstrained. \emph{Deployment efficiency} means the resulting scoring and control procedure has acceptable online computational cost relative to the alternatives it is compared against. In the framework studied here, decision alignment is built into the proposed target by construction; decision informativeness and deployment efficiency are both empirically investigated rather than assumed.

Within this perspective, the paper has three aims, each corresponding to one of the properties above. First, it formulates eviction as a candidate-level value-prediction problem that is decision-aligned by construction, instantiated as a finite-horizon eviction-value predictor. Second, it develops an empirical framework for testing whether this decision-aligned target is also informative and competitive in practice, comparing it against classical, predictive, robust, and learned reference policies, and against alternative candidate-level objectives, under a common replay protocol. Third, it diagnoses when and why the target is, or is not, informative enough to induce useful online eviction decisions, including the roles of horizon truncation, target degeneracy, continuation mismatch, and deployment cost. The optional fallback mechanism is retained as an unvalidated implementation extension, not as part of these three empirical aims. Taken together, these aims position the work as a methodological and empirical study of whether decision alignment alone is sufficient for effective cache control.

The scope of the paper is intentionally focused on standard online paging with fixed cache capacity and unit miss cost. We do not present a competitive-ratio guarantee for the learned policy, nor claim universal superiority over alternative learning-augmented formulations. The contribution is instead a decision-aligned supervision target, a candidate-scoring framework built on that target, and a direct empirical test of whether decision alignment translates into decision informativeness and deployment efficiency.
...
\begin{enumerate}
    ...
    \item We directly test the premise that the proposed eviction-loss target is better aligned with the eviction decision than alternative finite-horizon objectives. A full-pipeline ablation against reuse-distance, next-arrival, and pairwise-preference supervision finds the eviction-loss instantiation of \texttt{evict\_value\_v1} worst or tied-worst of the four. A subsequent matched common-model control, holding architecture, features, folds, and seed fixed and changing only the training objective, does \emph{not} support the further claim that the eviction-loss training objective itself is the cause: eviction-loss is nominally best by aggregate misses and is not materially worse than the alternatives.

    \item We provide a mechanistic diagnosis combining an exact-oracle replay, a target-degeneracy audit, a learned-versus-exact agreement analysis, and a tie-aware follow-up that varies how exact minimizers are resolved. This evidence disfavors gross model-fitting failure and insufficient training data as primary explanations. The horizon-4 target is highly action-underdetermined: every measured non-LRU exact-oracle decision has multiple optimal actions. The previously reported deterministic exact-oracle deficit versus LRU is therefore a tie-breaking confound, not proof that the exact target intrinsically performs worse than LRU. We further isolate, via a controlled causal ablation, the specific contribution of a mismatch between the label-construction continuation rule and the policy's deployed trajectory, finding a real but partial and family-dependent effect, and show through a companion distribution-shift-correction experiment that reducing a generic measure of this mismatch does not by itself improve online performance.

    \item We report a controlled, repeated-measurement wall-clock timing characterization showing the learned policy's per-decision cost is orders of magnitude higher than the lightweight baselines, and combine this evidence with the negative comparison result above to identify concrete lessons for future learning-augmented eviction-target design, rather than presenting a demonstrated deployment-ready replacement policy.
\end{enumerate}
...
A second relevant line comes from learned cache-replacement systems that use future-derived supervision to guide eviction. PARROT learns to imitate Belady-style eviction behavior from trace-derived oracle decisions, while LRB and Raven use Belady-guided future-access prediction to drive learned replacement decisions \cite{liu2020parrot,song2020lrb,hu2022raven}. Mockingjay moves beyond binary Belady-style labels by learning finer-grained reuse-time estimates for eviction ordering \cite{shah2022mockingjay}. HALP is especially relevant because it uses candidate-level preference learning from future re-access outcomes rather than direct action imitation \cite{song2023halp}. 3L-Cache combines a lightweight learned scorer with bidirectional candidate sampling and adaptive training controls \cite{zhou2025threelcache}, while CACHEUS combines learned expert selection with adaptive weighting across complementary eviction experts \cite{rodriguez2021cacheus}. LRB, 3L-Cache, CACHEUS, and HALP are included as direct empirical comparisons in Section~\ref{subsec:major1_comparison}, with implementation-fidelity caveats disclosed there and in Section~\ref{subsec:limitations}. These methods establish that candidate-level or future-derived learning is an existing design space; our narrower distinction is the particular finite-horizon counterfactual downstream-miss target, not candidate-level learning itself.

This distinction matters because, in many prior formulations, the learned quantity remains an intermediate proxy that must still be translated into an eviction action. Such proxies include oracle action labels, reuse-distance estimates, next-arrival forecasts, and pairwise candidate orderings \cite{liu2020parrot,shah2022mockingjay,hu2022raven,song2023halp}. By contrast, our formulation assigns each available candidate a counterfactual finite-horizon downstream-miss quantity. This is structurally decision-aligned, but structural alignment does not imply decision informativeness or good online performance. The framing is conceptually related to decision-focused learning and predict-then-optimize work \cite{elmachtoub2022smart_predict_optimize,wilder2019decision_focused_learning,shah2022lodl}; these foundations motivate the distinction between predictive accuracy and downstream decision quality, but do not establish a cache-specific theory for our target. Predict-then-optimize theory also treats multiple optimal solutions as a form of degeneracy and studies distance-to-degeneracy or margin concepts \cite{elbalghiti2022generalization}; we use that terminology only as a conceptual connection, not as a theorem about our discrete replay process.

The paper is also related to recent finite-horizon predictive approaches such as MUSTACHE, which uses multi-step-ahead page prediction to improve eviction decisions \cite{tolomei2022mustache}. That work is particularly relevant because it introduces an explicit forecasting horizon. Our method differs in what is supervised: rather than predicting future pages or arrival times and then deriving eviction decisions from those forecasts, we train directly on a finite-horizon eviction-cost target defined at the candidate level. MAT makes a complementary efficiency tradeoff by using a heuristic tail filter to restrict when learned predictions are needed; its authors report eight production workloads and a reduction from 63 to 2 predictions per eviction in the abstract, but we use it here only for its qualitative candidate-filtering distinction because it is an arXiv preprint rather than a peer-reviewed cache-systems publication \cite{yang2023mat}.

Recent systems also emphasize learning granularity and execution cost. GL-Cache learns at the group level to amortize inference across objects, while LAH/S4-FIFO learns cache-level parameters for a static heuristic and separates occasional control-plane learning from the high-speed data plane \cite{yang2023glcache,xia2026lah,li2026merlin}. Merlin provides a further recent example of fine-grained workload characterization combined with explicit separation of component responsibilities \cite{li2026merlin}. These approaches reinforce a practical systems lesson relevant to our results: learned caching must jointly provide useful decision information, favorable online miss behavior, and acceptable hot-path cost. S3-FIFO and SIEVE provide important non-ML reference points for the same efficiency pressure: both use simple FIFO-style structures to reduce per-hit or per-eviction overhead while retaining competitive cache efficiency \cite{yang2023s3fifo,zhang2024sieve}. Our empirical comparison includes SIEVE and FIFO-Reinsertion as lightweight baselines.
...
\end{equation}
where the features summarize the current request, the cache state, and candidate-specific predictive context. In the current repository, these features are organized into a small number of interpretable groups, including request-side features, candidate-side recency and reuse features, predictor--LRU comparison features, cache-state summary features, disagreement features, and recent-activity features. When predictive metadata are available, they are incorporated through these feature groups rather than treated as hard instructions. Table~\ref{tab:evict-value-feature-groups} in Appendix~\ref{sec:appendix_feature_groups} summarizes the feature families used by the scorer.
...
\subsection{Guarded Fallback Mechanism (Unvalidated Extension)}
\label{subsec:guarded_fallback_mechanism}

The repository also implements an optional, lightweight guard that can temporarily delegate eviction control to a conservative fallback policy when recent online behavior suggests that the learned scorer's decisions are locally unsafe. The guard monitors repeated \emph{very-early-return} evictions -- cases in which an item evicted by the learned scorer is requested again within a short window, treated as a signal that an item with immediate reuse value may have been removed -- and, once a sliding-window count of such suspicious evictions exceeds a threshold, temporarily switches control to a conservative fallback policy (e.g., least-recently-used eviction or a marker-style policy) for a bounded duration before returning to the base learned policy. The trigger signal, window and threshold parameters, fallback duration, and fallback choice are heuristic implementation decisions, not a theorem-backed robustness guarantee \cite{wei2020better_simpler_lac,chledowski2021robust_lac_experimental}.

We treat this guard as an unvalidated implementation extension rather than as a demonstrated component of the present results: its end-to-end effect has not been measured in this study, it is not used to support any quantitative claim in this paper, and it is not part of the three empirical aims stated in Section~\ref{subsec:objective_scope} (see also Limitations). The full formal specification of the guard is given in Appendix~\ref{sec:appendix_fallback_formalism} for conceptual completeness.
...
\caption{Conceptual overview of the eviction-value workflow. Offline replay is used to construct candidate-level training examples and finite-horizon loss targets, a supervised scorer is fitted to estimate decision-time eviction loss, and the online policy evicts the candidate with minimum predicted loss. The figure also depicts an optional, unvalidated fallback layer that can temporarily delegate control to a conservative baseline when repeated local signals indicate unsafe learned decisions; this fallback layer is not evaluated in the present study and is shown for conceptual completeness only (Section~\ref{subsec:guarded_fallback_mechanism}, Appendix~\ref{sec:appendix_fallback_formalism}).}
...
\small
\begin{tabular}{p{2.0cm} p{3.2cm} p{4.0cm} p{2.6cm}}
\hline
\textbf{Family} & \textbf{Original domain/source} & \textbf{Transformation} & \textbf{Role} \\
\hline
BrightKite & Location check-ins; SNAP Brightkite data \cite{cho2011friendship_mobility} & Venue ID becomes the requested item; original record order is retained. & Non-cache locality stress stream. \\
Citi Bike & Bike-share trip histories \cite{citibike_system_data} & Start-station ID becomes the requested item; original trip order is retained. & Non-cache locality stress stream. \\
CloudPhysics & Production block I/O collection \cite{cachemon_open_source_cache_dataset} & LBN is mapped to a logical page with 4096-byte page size; read-only records are used. & Storage-derived cache workload. \\
MetaCDN & Meta CDN request traces via CacheLib/cacheMon \cite{cachemon_open_source_cache_dataset} & Manifest-selected object IDs are replayed as request items. & Cache-derived workload. \\
MetaKV & Meta key-value cache traces via CacheLib/cacheMon \cite{cachemon_open_source_cache_dataset} & Manifest-selected keys are replayed as request items. & Cache-derived workload. \\
Twemcache & Twitter production cache traces \cite{twitter_cache_trace,yang2020twitter_cache_clusters} & Manifest-selected cluster/sample records; anonymized keys become request items. & Production cache workload. \\
Wikimedia & Wikimedia pageviews \cite{wikimedia_pageviews} & Pageview object IDs become request items; the evaluated artifact is a pageview-derived proxy, not a native CDN trace. & Public event-stream proxy. \\
\hline
\end{tabular}
\end{table}

The repository evaluates heterogeneous sources, but they are not interchangeable cache traces. BrightKite and Citi Bike are real-world check-in and trip-event streams transformed into request sequences; CloudPhysics, MetaCDN, MetaKV, and Twemcache are cache/storage-derived sources; and the Wikimedia artifact used here is a pageview-derived request proxy rather than a native CDN trace \cite{cho2011friendship_mobility,citibike_system_data,cachemon_open_source_cache_dataset,twitter_cache_trace,wikimedia_pageviews}. We state these distinctions explicitly in Table~\ref{tab:trace_families} above and treat the non-cache event streams as locality-stress workloads, not as evidence of production cache behavior.
...
This horizon effect reflects a specific tradeoff in label quality, distinct from the causal interpretation of online underperformance given in Section~\ref{subsec:mechanistic_diagnosis}: a short horizon ties the eviction-value label closely to the immediate consequence of the scored decision, while a long horizon allows the label to absorb increasing variance from the continuation policy's own subsequent choices, effects that are downstream consequences of later decisions rather than of the eviction being scored. Beyond the regret and Top-1 metrics in Table~\ref{tab:evict-value-ablation}, validation MAE, RMSE, and mean regret-vs-oracle also degrade monotonically as $H$ increases from 4 to 8 to 16, for every model class evaluated. This pattern holds not only in aggregate but within every individual trace family represented in our validation sample (BrightKite, CloudPhysics, MetaCDN, Twemcache, and Wikimedia pageviews), suggesting that the advantage of shorter horizons in this framework is not specific to any one workload among those evaluated. We have not yet evaluated whether this pattern extends to the remaining trace families in our full dataset or whether it interacts with cache capacity; we flag both as open questions for future work rather than claim to have resolved them.

A historical, single-split end-to-end evaluation that predates the corrected protocol below, and that additionally covers four reference policies not included in the corrected comparison (PredMk, T\&D, BO/LRU, REST), is reported in Appendix~\ref{sec:appendix_historical_e2e} for completeness. It is explicitly historical exploratory analysis, not primary performance evidence: the corrected, leakage-free comparison in the remainder of this section is the primary evidence for \texttt{evict\_value\_v1}'s comparative performance in this paper.
...
The proposed eviction-loss instantiation of \texttt{evict\_value\_v1} is the worst of the four full-pipeline alternatives, and it is the single worst objective within every one of the seven trace families individually (in the degenerate Wikimedia family, all four objectives tie at full misses), not merely on average. This contradicts the premise, stated in Section~\ref{subsec:eviction_value_framework}, that decision-aligned finite-horizon eviction-loss supervision is inherently better matched to the eviction decision than these indirect alternatives \emph{in the deployed \texttt{evict\_value\_v1} pipeline}. We report that pipeline result plainly.

That comparison, however, does not isolate the training objective as the sole causal factor: other pipeline differences can still co-vary with the label. To test the narrower hypothesis that the eviction-loss \emph{training objective itself} is responsible for the poor learned-policy performance, we run a matched common-model control on the same 21 (family, capacity) cells. Architecture, features, folds, windows, and seed are held fixed; only the supervision objective changes, and the pairwise objective uses a corrected orientation that trains on the intended preference label rather than candidate-identifier order. Table~\ref{tab:common-model-v2} reports the result.

\begin{table}[!htbp]
\centering
\caption{Matched common-model objective control (V2): identical folds, windows, features, architecture, and seed; only the training objective changes. 21 primary cells, total misses (lower is better).}
...
Under this matched control, eviction-loss is nominally best by total misses and by macro mean miss ratio; pairwise preference is second by totals ($\approx 0.9\%$ behind) but strongest by per-cell rank. Eviction-loss is not materially worse than the alternatives. The matched experiment therefore does \emph{not} support the hypothesis that the eviction-loss training objective itself explains the poor performance of the full learned policy. The Table~\ref{tab:objective-comparison} pipeline ranking remains a valid description of \texttt{evict\_value\_v1} as deployed; it should not be read as an isolated objective-causality result. Section~\ref{subsec:mechanistic_diagnosis} investigates the remaining, stronger supported diagnosis: the horizon-4 target is highly action-underdetermined, so downstream selection among many exact minimizers dominates online behavior.

\subsection{Mechanistic Diagnosis of the Eviction-Loss Target}
\label{subsec:mechanistic_diagnosis}

Sections~\ref{subsec:major1_comparison} and~\ref{subsec:objective_comparison} establish that \texttt{evict\_value\_v1} underperforms both external baselines and, in its full pipeline, alternative supervision objectives, while the matched common-model control in Table~\ref{tab:common-model-v2} shows that this is not explained by the eviction-loss training objective alone. This section asks \emph{why}, using targeted diagnostics run under the same matched protocol and the same 21 (family, capacity) cells. We organize the diagnosis as a sequence of questions, each answered by a specific measurement rather than by argument.
...
\textbf{Is the target itself useful, even if perfectly optimized?} We first replay the exact horizon-4 oracle under a deterministic tie convention (minimum candidate identifier among exact minimizers), rather than a learned approximation of it. Across the same 21 cells, that deterministic oracle has a mean miss ratio of $0.770090$, compared to $0.672769$ for LRU -- it loses to LRU on 18 of 21 cells, ties on 3, and never wins, with a mean cell-level disadvantage of $+0.0973$ and $81{,}750$ additional total misses ($646{,}876$ versus $565{,}126$). This shows that the \emph{deterministic target-plus-tie-resolution instantiation} is not a good online eviction policy. It is not, by itself, a statement about every policy that could resolve the same optimal sets differently.

A follow-up tie-aware control therefore varies only how exact minimizers are chosen, using the same 21 cells and the same exact H4 minimizer sets. Every measured non-LRU oracle decision has multiple optimal actions (\texttt{fraction\_tied\_decisions}${}=1.0$ on all $168$ tie-aware rows; mean fraction of all-candidates-tied decisions $\approx 0.649$; mean optimal-set fraction $\approx 0.991$; the three Wikimedia cells are fully degenerate). Choosing the LRU-most candidate among exact minimizers never loses to LRU: it wins on $16$ of $21$ cells, ties on $5$, and incurs $413$ fewer total misses than LRU ($564{,}713$ versus $565{,}126$). Choosing the MRU-most minimizer reproduces a deficit similar to the deterministic oracle (0 wins / 3 ties / 18 losses; $+89{,}135$ total misses). Uniform random choice among minimizers, over five fixed seeds, still usually loses to LRU (7 wins / 15 ties / 83 losses over $105$ rows) but usually beats the deterministic oracle (90 wins / 15 ties / 0 losses). The previously reported deterministic exact-oracle deficit versus LRU is therefore a tie-breaking confound. It does \emph{not} establish that the exact H4 target intrinsically performs worse than LRU. What the data do establish is that the target is highly action-underdetermined, so downstream selection among many exact minimizers can change the online result from a large deficit to a small advantage over LRU.

\textbf{Why is the target so underdetermined?} We audit the target's decision-level structure independently of any one tie rule. Across the same 21 cells, no decision ever has a unique optimal eviction candidate (unique-winner fraction $0.0$); every decision has multiple tied-optimal candidates (multiple-optimum fraction $1.0$); in $76.4\%$ of decisions in the strict-preference diagnostic every resident candidate is exactly tied for optimal; and on average $99.5\%$ of resident candidates fall within the optimal set (mean optimal-set fraction $0.9949$). These figures describe the same qualitative degeneracy as the tie-aware replay above. The horizon-4 target almost never provides a strict, informative preference among the cached candidates, so the specific eviction choice a learned or exact policy makes among the many tied-optimal candidates is effectively unconstrained by the target, yet still determines downstream performance. A companion diagnostic quantifies an underlying observability limit: across $41{,}615{,}776$ candidate-residency observations at $565{,}126$ decision points, the probability that an item's true reuse time exceeds the 4-step horizon is $0.9939$ overall and $0.9793$ even conditioned on the item eventually being reused. The horizon-4 window observes only a small fraction of the reuse behavior that determines whether an eviction was actually costly, which is consistent with -- and offers a mechanistic explanation for -- the observed target degeneracy: most candidates' true consequences fall outside the window the target can see, so the target frequently cannot distinguish them.

\textbf{Summary.} A separate learning-curve check across five training-set fractions from 1\% to 50\% of the available candidate-level examples shows no material improvement in target-quality metrics with additional data, disfavoring insufficient training data (Section~\ref{subsec:main_results}'s offline ablation already selects the best-performing configuration within this data regime). Taken together with the agreement, oracle, degeneracy, and tie-aware diagnostics above, these results disfavor both insufficient training data and gross model-fitting failure. They also fail to support the claim that the eviction-loss training objective itself is the culprit (Table~\ref{tab:common-model-v2}). The strongest supported diagnosis is that the horizon-4 target is highly action-underdetermined, so selection and deployment behavior inside a large optimal set matter substantially. Horizon truncation relative to typical reuse timescales is a consistent mechanistic background for that degeneracy. This does not, by itself, prove that tie degeneracy explains every aspect of learned-policy underperformance. Section~\ref{subsec:continuation_ablations} next examines a secondary, partial contributor: the mismatch between the label-construction continuation rule and the policy's own deployed trajectory.

\subsection{Continuation-Mismatch and Distribution-Shift Ablations}
\label{subsec:continuation_ablations}

Section~\ref{subsec:mechanistic_diagnosis} establishes action-underdetermined (tie-degenerate) supervision, and the sensitivity of exact-oracle replay to how minimizers are resolved, as the strongest supported diagnosis. This section reports two further controlled experiments testing whether a learnable, decision-aligned offline target would translate into a policy that underperforms strong baselines online because of the specific continuation-rule assumption used to build the supervision target, or because of a broader mismatch between the training-state distribution and the deployment-time trajectory. Both are controlled causal tests of hypotheses that were speculative in an earlier version of this study: that the eviction-loss label's fixed LRU-style continuation assumption diverges from the policy's own recursive, non-LRU deployment trajectory, and that this divergence contributes to the observed gap.

We test the continuation-mismatch hypothesis directly with a three-arm causal ablation under the leave-one-family-out protocol, isolating exactly one change between arms. \emph{C0} replays LRU end-to-end (the reference). \emph{C1} trains a policy $\pi_1$ on labels built under LRU continuation (matching Eq.~\eqref{eq:eviction_loss} as originally specified) and deploys $\pi_1$ recursively. \emph{C2} trains a second policy $\pi_2$ on labels built under a corrected continuation rule -- the frozen $\pi_1$'s own decisions, rather than LRU -- and deploys $\pi_2$ recursively; this is the only change between C1 and C2. Across the 21 (family, capacity) cells, C2 improves over C1 in 13 cells, ties in 3 (the degenerate Wikimedia cells), and worsens in 5, with a macro mean miss-ratio improvement of $-0.0102$; aggregate misses move from $601{,}569$ (C1) to $592{,}970$ (C2), against a $565{,}126$-miss LRU reference (C0). The improvement is not uniform: one cell, BrightKite at capacity 32, regresses sharply ($+0.2433$). The present experiments do not isolate the mechanism behind this specific reversal, and we do not speculate about it here; it remains an important counterexample that by itself prevents a claim of uniform benefit. We therefore classify the continuation-mismatch hypothesis as \emph{partially supported and regime-dependent}: correcting the continuation-label mismatch helps in most but not all family/capacity combinations, and is a real but secondary and non-uniform contributor to the gap documented in Sections~\ref{subsec:major1_comparison}--\ref{subsec:mechanistic_diagnosis}, not a full explanation of it.
...
Viewed in this way, the offline ablation indicates that replay-based candidate supervision provides a viable route for translating predictive context into eviction decisions; tree-based scorers consistently outperform the linear ridge baseline across replay horizons, and shorter replay horizons yield the strongest regret values within the current artifact set, consistent with the interpretation that longer horizons incorporate harder-to-model downstream interactions (Section~\ref{subsec:main_results}). A candidate-level downstream-loss target of this kind remains a proxy, but it is defined directly at the decision point, which gives it decision alignment by construction; whether that alignment is also informative and efficient enough for competitive online use is exactly the question the remainder of this paper tests.

At the same time, the interpretation of these results should remain cautious, and the end-to-end evaluation in Appendix~\ref{sec:appendix_historical_e2e} sharpens rather than removes that caution. The replay-based loss studied here is finite-horizon and local rather than globally optimal, and the offline scorer-selection ablation does not by itself establish superiority in full end-to-end online replay. The explicit end-to-end evidence we report directly answers that question, and the answer is negative: across all three evaluated capacities, \texttt{evict\_value\_v1} incurs more replay misses than LRU, SIEVE, and FIFO-Reinsertion, with the gap widening sharply and non-monotonically at capacity 128. Likewise, the broader framework includes guarded variants whose end-to-end effect remains unmeasured in the present study (see Limitations). The formulation is plausible and directly aligned with the eviction decision by construction; the experiments in this paper test whether that structural alignment is, by itself, sufficient for competitive online behavior. Current evidence does not establish competitive online utility: it does not support a claim of downstream competitiveness, robustness across workload regimes, or effective fallback control in deployment, and the end-to-end evidence weighs against competitiveness at every capacity evaluated so far.

Taken together, the artifact-backed evidence supports a clear but negative conclusion about the present instantiation of \texttt{evict\_value\_v1}, together with a mechanistically informed -- and now more carefully bounded -- account of why. Finite-horizon candidate-level scoring, as a general framework for translating predictive context into eviction decisions, is directly decision-aligned by construction and was learnable in the offline ablation, where short-horizon nonlinear scorers were the strongest configuration for the eviction-loss target specifically (Section~\ref{subsec:main_results}). The full \texttt{evict\_value\_v1} pipeline does not outperform any of seven classical or learned baselines (Section~\ref{subsec:major1_comparison}), and in that pipeline eviction-loss is the worst of four supervision objectives (Table~\ref{tab:objective-comparison}). A matched common-model control, however, shows that this pipeline ranking is not explained by the eviction-loss training objective itself (Table~\ref{tab:common-model-v2}). The strongest supported diagnosis is instead that the horizon-4 target is highly action-underdetermined: every measured non-LRU exact-oracle decision has multiple optimal actions, so downstream selection among minimizers can change online performance from a large deterministic-oracle deficit versus LRU to a small LRU-within-minima advantage (Section~\ref{subsec:mechanistic_diagnosis}). Continuation-label mismatch contributes a real but partial, regime-dependent secondary effect (Section~\ref{subsec:continuation_ablations}), and generic distribution-shift correction offers no remedy on its own. We characterize \texttt{evict\_value\_v1}, in its present form, as a decision-aligned supervision study whose deployed policy is unsuccessful and whose exact target is informative only after a secondary rule is supplied, rather than as a practically superior online policy. LRU, SIEVE, FIFO-Reinsertion, LRB, 3L-Cache, CACHEUS, and HALP all remain baselines the proposed method does not currently outperform under matched evaluation. Future work on this framework should treat horizon design, target informativeness, and explicit tie resolution as primary design questions, not assume that model capacity, training data volume, or a change of training objective alone will solve the problem.
...
Given the negative miss-ratio comparison in Section~\ref{subsec:major1_comparison} and the timing evidence in Section~\ref{subsec:overhead_scalability}, we state directly what practical value, if any, the present study offers, rather than leaving this question implicit. As currently instantiated, \texttt{evict\_value\_v1} is not a candidate for replacing LRU, SIEVE, FIFO-Reinsertion, or the evaluated learned baselines in latency- or throughput-sensitive deployments. We do not identify a deployment scenario in which this specific cost/performance combination is currently justified, and we do not claim one.

The value of the present study is instead diagnostic and methodological: it provides a controlled empirical test of a specific supervision-target design (finite-horizon, candidate-level eviction loss) that is representative of a broader family of approaches in learning-augmented caching, and it isolates, through Sections~\ref{subsec:objective_comparison}--\ref{subsec:discussion_analysis}, which aspects of that design are, and are not, responsible for its failure to outperform simpler alternatives. This kind of evidence is useful precisely because it is negative and mechanistically constrained rather than merely negative: it identifies action-underdetermined supervision and tie-sensitive exact-oracle replay, not the eviction-loss training objective, model capacity, training data volume, or gross prediction error, as the primary supported barrier, which directly informs what a future redesign of this target family would need to change (Section~\ref{subsec:future_research}). We see two narrower settings where the present framework's cost profile could plausibly become acceptable in future work, not in its current form: offline or batch-oriented research and analysis pipelines where decision quality, not per-decision latency, is the binding constraint and scoring cost can be amortized outside the request path; and as a supervision-objective research tool for evaluating alternative finite-horizon targets before any online deployment is considered, which is in fact how it is used within this paper (Section~\ref{subsec:objective_comparison}). We do not present either as a validated production use case.

Recent cache systems illustrate why this distinction matters. Rather than placing a large learned scorer on every eviction path, recent designs reduce cost through candidate filtering at the tail (MAT), group-level inference that amortizes signals across objects (GL-Cache), cache-level parameter learning separated from the data plane (LAH/S4-FIFO), or carefully structured low-overhead sampling and characterization (3L-Cache and Merlin) \cite{yang2023mat,yang2023glcache,xia2026lah,zhou2025threelcache,li2026merlin}. Strong non-ML designs such as S3-FIFO and SIEVE provide a complementary reminder that simple queue structures can deliver useful efficiency and scalability without learned per-candidate inference \cite{yang2023s3fifo,zhang2024sieve}. We therefore frame the practical lesson narrowly: a learned cache replacement design must jointly provide decision information, favorable online miss behavior, and acceptable hot-path cost. The present implementation demonstrates none of those three conditions strongly enough for deployment, but it provides a controlled test of how a seemingly decision-aligned target can fail them.
...
The remaining findings are cautionary and, taken together, negative for the present \texttt{evict\_value\_v1} instantiation, but they are directly and mechanistically explained rather than left as an unexplained empirical gap. End-to-end online replay evaluation, both under the original single-split protocol (capacities 32, 64, and 128; Section~\ref{subsec:end_to_end_capacities}) and under a corrected, leakage-free protocol matched against seven classical and learned baselines including LRB and 3L-Cache (Section~\ref{subsec:major1_comparison}), shows that \texttt{evict\_value\_v1} does not outperform LRU, SIEVE, FIFO-Reinsertion, LRB, 3L-Cache, CACHEUS, or HALP. A full-pipeline ablation against three alternative finite-horizon supervision objectives (Section~\ref{subsec:objective_comparison}) shows that the eviction-loss instantiation is the worst or tied-worst of the four in every trace family, while a matched common-model control shows that this ranking is not explained by the eviction-loss training objective itself. A mechanistic diagnosis (Section~\ref{subsec:mechanistic_diagnosis}) finds severe target degeneracy at the tested horizon and shows that the deterministic exact-oracle deficit versus LRU is a tie-breaking confound: LRU-within-minima never loses to LRU. A controlled causal ablation (Section~\ref{subsec:continuation_ablations}) finds that mismatch between the label-construction continuation rule and the policy's deployed trajectory is a real but partial, family-dependent secondary contributor, and a companion distribution-shift-correction experiment shows that reducing a generic measure of this shift does not by itself improve online performance. Finally, a controlled timing campaign shows the learned policy's per-decision cost is orders of magnitude higher than the lightweight baselines (Section~\ref{subsec:overhead_scalability}). Capacity-256 evaluation and guarded-deployment evidence remain left to future work.

\subsection{Implications of the Proposed Approach}
\label{subsec:implications}

The proposed approach, and its negative outcome, both have implications for learning-augmented cache design. The first is methodological: in cache replacement, the supervision target can be as important as the predictive signal itself, but proximity to the eviction decision is not, by itself, sufficient for a target to be effective. The eviction-loss target studied here is defined directly at the candidate level, yet the deployed \texttt{evict\_value\_v1} pipeline is outperformed by less directly loss-aligned alternatives (Table~\ref{tab:objective-comparison}) and, more importantly, the exact target is highly action-underdetermined at the tested horizon (Section~\ref{subsec:mechanistic_diagnosis}). A matched common-model control shows that this is not because the eviction-loss training objective is itself a uniquely harmful label (Table~\ref{tab:common-model-v2}). The present study therefore suggests a more specific design lesson than ``define the target close to the decision'': a finite-horizon target must also be checked for decision-level informativeness (whether it actually distinguishes candidates), for how ties among minimizers are resolved, and for horizon adequacy relative to the workload's characteristic reuse timescale. In the terms introduced in Section~\ref{subsec:objective_scope}, decision alignment alone is not sufficient; decision informativeness and deployment efficiency (Sections~\ref{subsec:overhead_scalability}--\ref{subsec:practical_significance}) must also hold. This does not, by itself, imply that candidate-level or decision-aligned learning is unworkable for cache replacement in general: the negative result is a property of this specific target, horizon, and deployed pipeline, not a general failure of candidate-level supervision.

A second implication is that robust learning-augmented caching need not be expressed only through global trust calibration or expert switching: in the present implementation, predictive information is incorporated through candidate-level scoring rather than through a global trust signal. The codebase also includes an optional guard-style fallback extension (Section~\ref{subsec:guarded_fallback_mechanism}, Appendix~\ref{sec:appendix_fallback_formalism}); it is not used to support any quantitative claim in this revision, so the main evidence for this implication concerns the candidate-level scoring formulation and its end-to-end replay behavior, not a validated fallback mechanism.
...
Third, the end-to-end evidence against \texttt{evict\_value\_v1} is now extensive and, unlike in an earlier version of this study, mechanistically explained rather than merely observed. The single-split evaluation in Section~\ref{subsec:end_to_end_capacities} shows negative results at every evaluated capacity but relies on a model with documented train/test overlap (disclosed in that section); the corrected, leakage-free evaluation in Section~\ref{subsec:major1_comparison} confirms the same qualitative conclusion against seven classical and learned baselines. The full-pipeline objective ablation in Section~\ref{subsec:objective_comparison} shows the eviction-loss instantiation of \texttt{evict\_value\_v1} is outperformed by alternative targets in that pipeline, but the matched common-model control shows that this is not because the eviction-loss training objective itself is uniquely harmful. Sections~\ref{subsec:mechanistic_diagnosis} and~\ref{subsec:discussion_analysis} identify severe target degeneracy -- equivalently, a highly action-underdetermined exact target -- as the leading supported diagnosis, and show that deterministic exact-oracle replay is tie-sensitive: it must not be read as proof that the exact target intrinsically loses to LRU. Continuation-label mismatch remains a real but partial, family-dependent secondary contributor (13 of 21 cells improve under a corrected continuation rule, but one cell regresses sharply), while generic distribution-shift correction offers no remedy on its own (a DAgger-style intervention reduces the measured shift metric in 16 of 21 cells while online misses worsen in 16 of 21). These experiments constrain interpretation rather than proving one complete causal mechanism: they do not isolate every downstream cause of learned-policy underperformance, and they remain bounded by the specific horizon ($H=4$), continuation-correction method (one-step, frozen-policy relabeling), and shift-reduction method (one-step DAgger) tested. A different horizon, a multi-round continuation correction, or a decision-targeted (rather than generic) shift-reduction method are not ruled out and are left to future work (Section~\ref{subsec:future_research}). Capacity 256 has not been evaluated end-to-end under either protocol.
...
Fifth, empirical conclusions in learning-augmented caching remain sensitive to workload characteristics, cache capacities, and replay protocol choices. The seven families are deliberately heterogeneous, but BrightKite and Citi Bike are non-cache event streams transformed into request sequences, Wikimedia is a pageview-derived proxy, and the cache/storage-derived families are themselves sampled or transformed by their source formats. The 50,000-request windows and unit-size paging abstraction also do not reproduce the scale, byte costs, or admission semantics of production caches. Baseline implementation fidelity is not uniform: LRB and 3L-Cache are documented independent reimplementations, CACHEUS uses the authors' decision engine through an adapter, and HALP is a lower-fidelity supporting adaptation. Comparisons are descriptive paired replay results over the 21 family-capacity cells, not inferential claims about a population of independent traces; the learned evaluations also use fixed seeds. The current results should therefore be read as evidence bearing directly on the tested target, horizon, workload transformations, and evaluated implementations, rather than as a claim ruling out every possible finite-horizon candidate-level formulation.

Sixth, the baseline-specific implementation-fidelity caveats for the matched-protocol comparison (batch-size, provenance, and reimplementation caveats for 3L-Cache, CACHEUS, and HALP respectively) are detailed once, in full, in Section~\ref{subsec:major1_comparison}'s implementation-fidelity caveats; they are separate from, and do not affect, the matched-evaluation-protocol dimensions themselves. Relatedly, \texttt{evict\_value\_v1}'s reported runtime (Section~\ref{subsec:overhead_scalability}) is a single, non-repeated measurement, not part of the controlled 5-repetition timing campaign covering LRU, FIFO-Reinsertion, SIEVE, and HALP; a directly comparable repeated-measurement timing campaign for \texttt{evict\_value\_v1} was judged computationally infeasible within this study's compute budget given its substantially higher per-decision cost, and should not be conflated with the four-policy controlled campaign.
...
Several research directions follow from this study's negative but mechanistically constrained result. First, and most directly motivated by Section~\ref{subsec:mechanistic_diagnosis}, future work should treat explicit tie resolution and target informativeness as first-class design questions: adaptive or longer replay horizons, terminal-value correction that extends the label beyond the truncation point, or uncertainty-aware and soft decision rules that do not require a strict preference to exist among tied-optimal candidates. The LRU-within-minima result shows that a recency secondary rule among exact minimizers can already change online behavior substantially; it does not, by itself, yield a competitive learned policy. Second, the continuation-mismatch finding in Section~\ref{subsec:continuation_ablations} is regime-dependent and based on a single-round correction; multi-round or iterative continuation correction, and a decision-targeted (rather than generic) distribution-shift correction, are natural next steps given that the tested one-step DAgger-style intervention did not itself improve performance. Third, the candidate-centric framework should be extended beyond unweighted paging to weighted and general-caching models with variable object sizes and heterogeneous miss costs. Fourth, ranking-based or other alternative supervision remains an open design axis, but it should not be motivated by the disproven claim that the eviction-loss training objective is uniquely harmful under a matched model. Fifth, the optional guarded controller remains heuristic; future work should develop validated fallback policies, including a confidence- or uncertainty-based trigger informed by the target-degeneracy findings above, and evaluate them together with broader capacity sweeps including capacity 256. Sixth, more efficient inference (e.g., approximate or selectively invoked candidate scoring) would be required before the overhead documented in Section~\ref{subsec:overhead_scalability} could be reduced enough to reconsider the practical framing in Section~\ref{subsec:practical_significance}. Finally, where possible, partial theoretical guarantees for restricted candidate-scoring or guarded deployment settings remain open. We list these as concrete, evidence-motivated directions rather than as completed work; none of the interventions above, other than the specific tie-aware and common-model controls already reported, has been evaluated as a full redesigned policy in this study.
...
\section{Historical Single-Split End-to-End Evaluation}
\label{sec:appendix_historical_e2e}

This appendix reports a historical, single-split end-to-end online replay evaluation of \texttt{evict\_value\_v1} that predates the corrected, leakage-free evaluation in Section~\ref{subsec:major1_comparison}. It is retained here for completeness and because it is the only evaluation in this paper covering four reference policies not included in the corrected matched comparison (PredMk, T\&D, BO/LRU, and REST). It is explicitly historical exploratory analysis, not primary performance evidence: the corrected, leakage-free evaluation in Section~\ref{subsec:major1_comparison} is the primary evidence for \texttt{evict\_value\_v1}'s comparative performance in this paper.

\subsection{End-to-End Online Replay Evaluation Across Available Capacities}
\label{subsec:end_to_end_capacities}

In addition to the offline ablation in Section~\ref{subsec:main_results}, we report end-to-end online replay results for all eight policies in Table~\ref{tab:main_policy_families}, evaluated under the common simulator described in Section~\ref{subsec:experimental_setup} across the seven trace families in Table~\ref{tab:trace_families}, at 50{,}000 requests per trace. At the time of writing, this evaluation has been completed at cache capacities 32, 64, and 128 slots; capacity 256 has not yet been evaluated end-to-end and is not reported in this paper. We are explicit about this boundary because the capacity-128 result discussed below is itself non-monotonic relative to capacities 32 and 64, and we judged it more important to report and analyze the existing three-point trend honestly than to extend further computation into a larger capacity sweep before doing so.
...
\begin{table}[!htbp]
\centering
\caption{Historical single-split end-to-end online replay evaluation: mean misses across the seven trace families in Table~\ref{tab:trace_families} (50{,}000 requests/trace), at each evaluated cache capacity. Capacity 256 has not yet been evaluated end-to-end. This table predates the corrected leave-one-family-out protocol of Section~\ref{subsec:major1_comparison} and is retained for its coverage of PredMk, T\&D, BO/LRU, and REST.}
...
\begin{table}[!htbp]
\centering
\caption{\texttt{evict\_value\_v1} relative miss gap against LRU, by trace family and evaluated capacity, from the historical single-split evaluation of Appendix~\ref{sec:appendix_historical_e2e}. Positive values indicate more misses than LRU (worse).}
...
Taken together with the offline ablation in Section~\ref{subsec:main_results}, the capacity-128 degradation documented above is broad-based across workload families rather than an artifact of a single outlier trace: four of the six non-degenerate families show a capacity-128 gap above $10\%$, reinforcing the negative end-to-end result already reported in this appendix. We discuss its likely cause in Section~\ref{subsec:discussion_analysis} and revisit it in Section~\ref{subsec:limitations}.

\section{Guarded Fallback Mechanism: Full Formalism}
\label{sec:appendix_fallback_formalism}

This appendix gives the full formal specification of the guarded fallback mechanism summarized in Section~\ref{subsec:guarded_fallback_mechanism}. As stated there, this mechanism is an unvalidated implementation extension: its end-to-end effect has not been measured in this study, and it is not used to support any quantitative claim in this paper.
...
This mechanism is intentionally lightweight and heuristic: trigger signal, thresholds, duration, and fallback choice are implementation decisions for controlled study, not a theorem-backed robustness guarantee \cite{wei2020better_simpler_lac,chledowski2021robust_lac_experimental}.

\section{Decision-Time Feature Groups}
\label{sec:appendix_feature_groups}
