...
Within this perspective, the paper has three aims. First, it formulates eviction as a candidate-level value-prediction problem in the unweighted paging setting. Second, it develops an empirical framework for evaluating this decision rule against classical, predictive, and robust reference policies under a common replay protocol. Third, it diagnoses when the target is informative enough to induce useful online eviction decisions, including the roles of horizon truncation, target degeneracy, continuation mismatch, and computational cost. The optional fallback mechanism is retained as an unvalidated implementation extension, not as a third empirical aim. Taken together, these aims position the work as a methodological and empirical study of decision-aligned cache control.
