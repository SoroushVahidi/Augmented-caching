...
This mechanism is intentionally lightweight: rather than calibrated uncertainty estimates, a separate adversarial detector, or a heavy expert-combination procedure, it uses an observable local failure pattern directly tied to eviction quality, so safety is assessed through concrete recent consequences rather than an abstract global trust score. It should therefore be interpreted as a practical empirical mechanism whose trigger signal, thresholds, duration, and fallback choice are implementation decisions for controlled study, not as a theorem-backed robustness guarantee; this positioning is consistent with the practical robustification literature in learning-augmented caching, where switching and fallback mechanisms limit the damage caused by unreliable predictive behavior \cite{wei2020better_simpler_lac,chledowski2021robust_lac_experimental}.
...
\begin{table}[!htbp]
\centering
\caption{Trace families relevant to the experimental study. The repository supports a broader workload collection than any single table should claim; reported results are restricted to the subset directly instantiated by the canonical manuscript artifacts.}
...
\begin{table}[!htbp]
\centering
\caption{Main policy families used in the empirical comparison, distinguishing stable reference baselines from the proposed learned eviction-value policy. A diagnostic-only blind-oracle variant used for internal sanity checks (not for the comparisons reported here) is excluded.}
